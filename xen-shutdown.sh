...
main () {
	...
	# Seconds until VM clean suspend times out (seconds until force shutdown initiated)
	vm_suspend_timeout=600
	...
	log_date "==============================================================================="
	log_date "Powerdown event recieved from NUT master, initiating emergency shutdown proceedure"
	...
	sleep 10
	while [ $(( SECONDS - start_time )) -lt $vm_suspend_timeout ]; do
		vm_uuids=( $(xe vm-list power-state=running is-control-domain=false --minimal | tr , "\n" ) )
		if [ ${#vm_uuids[@]} -eq 0 ]; then
			...
			log_date "Not all VMs suspended, continuing to wait..."
			sleep 10
		fi
	done
